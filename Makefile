...
all:	authz tts audio_ingestion audio_ingestion2 kaldi_asr deepspeech_asr nlp_app rasa_action_server

run:
	docker volume create --driver local --opt o=bind,uid=800,gid=1102 --opt type=none --opt device=${SOCKET_DIR} zmq_ipc_vol && \
	docker stack deploy -c compose/docker-compose-backend.yml chat_backend
	docker-compose -p chat_frontend -f compose/docker-compose-frontend.yml up -d

stop:
	docker-compose -p chat_frontend -f compose/docker-compose-frontend.yml down
	docker stack rm chat_backend

run_respeaker:
	docker volume create --driver local --opt o=bind,uid=800,gid=1102 --opt type=none --opt device=${SOCKET_DIR} zmq_ipc_vol && \
	docker stack deploy -c compose/docker-compose-backend.yml chat_backend
	docker-compose -p chat_frontend -f compose/docker-compose-frontend-respeaker.yml up -d

stop_respeaker:
	docker-compose -p chat_frontend -f compose/docker-compose-frontend-respeaker.yml down
	docker stack rm chat_backend
